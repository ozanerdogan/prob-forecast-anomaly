...
def main() -> None:
    ...
    all_cov = [c for c in df.columns if c != TARGET]  # 13 exogenous variables
    print(f"Covariates ({len(all_cov)}): {all_cov}")
    ...
    for split_name, arr in (("val", data.val), ("test", data.test)):
        ...
        rows = {}
        for c in range(1, x.shape[2]):  # skip channel 0 (target)
            xp = x.copy()
            xp[:, :, c] = x[perm, :, c]
        ...
        for n, v in sorted(rows.items(), key=lambda kv: -kv[1]["delta_crps"]):
            tag = "[T-türevi]" if v["is_temp_derived"] else ("[takvim]" if v["is_calendar"] else "[bağımsız]")
            print(f"  {n:18s} {tag:11s} dCRPS {v['delta_crps']:+.4f}  dRMSE {v['delta_rmse']:+.4f}")

    out_path = ROOT / "results" / "base" / "covariate_importance_full.json"
